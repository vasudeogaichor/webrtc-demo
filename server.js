...
function leaveMeeting(socket, meetingId) {
  ...
  if (meeting) {
    ...
    meeting.participants.forEach(peer => {
      peer.send(JSON.stringify({ type: 'participant-left', peerId: socket.peerId }));
    });
    ...
    switch (data.type) {
      case 'join-meeting':
        ...
        const existingPeers = meetings[meetingId].participants
          .filter(p => p !== socket)
          .map(p => p.peerId);
        socket.send(JSON.stringify({ type: 'existing-participants', peers: existingPeers, currentPeerId: peerId }));
        break;
      ...
      case 'candidate':
        forwardSignal({
          ...data,
          payload: { // Recreate the candidate object with all required properties
